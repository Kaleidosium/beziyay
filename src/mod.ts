...
const copySegment = function (segment) {
  return {
    ...
    constrain_to: segment.constrain_to == undefined
      ? undefined
...
const addToCurveImpl = function (curve, new_point) {
  ...
  if (
    getMagnitude(new_point, getLastPoint(last_segment)) < RADIAL_SIMPLIFICATION
  ) {
    return last_segment;
  }
  ...
  if (
    ...
    if (UpdateStatus.FAIL_CORNER == last_segment.update_status) {
      ...
      new_segment = initCurveSegment(last_point.x, last_point.y);
      new_segment.constrain_to = getUnitVector(
        getCurveEndTangent(last_segment),
      );
    }
...
const updateDistanceField = function (curve, current_point) {
  ...
  for (let i = 0; i < horiz_steps; i++) {
    ...
    for (let j = 0; j < vert_steps; j++) {
      ...
      if (curve.vdmap[x][y] == undefined) {
        ...
        curve.vdmap[x][y] = sumOfSquaresVec(curve.vdmap[x][y]) <
            sumOfSquaresVec(roundVec(current_val))
          ? curve.vdmap[x][y]
  ...
  const upperLeftPoint = subVec(current_point, {
    x: FIELD_RADIUS,
    y: FIELD_RADIUS,
  });
  const bottomRightPoint = addVec(current_point, {
    x: FIELD_RADIUS,
    y: FIELD_RADIUS,
  });
...
const getCurveEndTangent = function (curve) {
  ...
  return {
    x: 3 * (control_point.x - endpoint.x),
    y: 3 * (control_point.y - endpoint.y),
  };
};
...
const checkCorner = function (curveSegment, point) {
  ...
  if (SHOULD_USE_NEW_CORNER_FINDER) {
    tan = getUnitVector(
      subVec(getPointAlongCurve(curveSegment, 0.95), curveSegment.c3),
    );
  } else {
