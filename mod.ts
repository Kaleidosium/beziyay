...
export class Curve {
  ...
  #initCurveSegment(x: number, y: number): CurveSegment {
    const point = { x, y };
    return {
      c0: point, c1: point, c2: point, c3: point, error: 0,
    };
  }
  ...
  #copySegment(segment: CurveSegment): CurveSegment {
    return {
      c0: { ...segment.c0 }, c1: { ...segment.c1 },
      c2: { ...segment.c2 }, c3: { ...segment.c3 },
      constrain_to: segment.constrain_to ? { ...segment.constrain_to } : undefined,
  ...
  public addToCurve(coord: Coord): CurveSegment | undefined {
    ...
    if (getMagnitude(roundedCoord, this.#getLastPoint(segmentToUpdate)) < RADIAL_SIMPLIFICATION) {
    ...
    if (segmentToUpdate.update_status && segmentToUpdate.update_status !== UpdateStatus.SUCCESS) {
        ...
        if (newSegment) {
            ...
            segmentToUpdate = newSegment; // Target the newly added segment for the upcoming update
             ...
             console.warn("Unhandled update status before adding point:", segmentToUpdate.update_status);
    ...
    let updatedSegment = this.#updateDistanceField(segmentToUpdate, roundedCoord);
  ...
  #updateDistanceField(segment: CurveSegment, currentPoint: Coord): CurveSegment {
      ...
      // Check if perpVec is valid (magnitude > 0)
       if(isNaN(perpVec.x) || isNaN(perpVec.y)) {
            ...
            console.warn("Degenerate segment in updateDistanceField, points are identical.");
      ...
      const vertIncrement = vertSteps > 0 ? divVec(vertDiff, vertSteps) : {x: 0, y: 0}; // Avoid division by zero
      const horizDiff = subVec(b1, a1);
      const horizSteps = getDDASteps(horizDiff);
      const horizIncrement = horizSteps > 0 ? divVec(horizDiff, horizSteps) : {x: 0, y: 0}; // Avoid division by zero
      ...
      for (let i = 0; i < horizSteps; i++) {
          ...
          for (let j = 0; j < vertSteps; j++) {
              ...
              if (existingVal === undefined || sumOfSquaresVec(roundedVal) < sumOfSquaresVec(existingVal)) {
      ...
      const upperLeftPoint = subVec(currentPoint, { x: FIELD_RADIUS, y: FIELD_RADIUS });
      const bottomRightPoint = addVec(currentPoint, { x: FIELD_RADIUS, y: FIELD_RADIUS });
      ...
      for (let x = startX; x < endX; x++) {
          ...
          for (let y = startY; y < endY; y++) {
              ...
              if (existingVal === undefined || sumOfSquaresVec(val) < sumOfSquaresVec(existingVal)) {
                  this.#vdmap[x][y] = val;
              }
          }
      }
      ...
      while (true) {
          ...
          for (let i = 0; i < NUM_SAMPLE_POINTS; i++) {
              ...
              if (distVec.x === undefined || distVec.y === undefined || isNaN(distVec.x) || isNaN(distVec.y)) {
                  console.warn(`Invalid distance vector at t=${t}, point=`, pointOnCurve);
          ...
          // Apply forces (check for NaN forces)
           if(isNaN(f1.x) || isNaN(f1.y) || isNaN(f2.x) || isNaN(f2.y)) {
             console.warn("NaN force vector detected, stopping iteration for this segment.");
             // Restore original state as forces are invalid
             segment.c0 = ogControls.c0; segment.c1 = ogControls.c1;
             segment.c2 = ogControls.c2; segment.c3 = ogControls.c3;
             segment.error = ogControls.error; segment.constrain_to = ogControls.constrain_to;
  ...
  #getDistanceFromPolyline({ x, y }: Coord): Coord {
      ...
      if (y + 1 < column.length) {
        ...
        if (second !== undefined) {
            if (second.x !== undefined && !isNaN(second.x) && Math.abs(val.x) > Math.abs(second.x)) {
                val.x = second.x;
            }
            if (second.y !== undefined && !isNaN(second.y) && Math.abs(val.y) > Math.abs(second.y)) {
                val.y = second.y;
            }
        }
      }
